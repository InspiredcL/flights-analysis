...
# Crea y carga la tabla particionada de manera local, dataset.tabla = dsongcp.flights_raw
...
bq --project_id $PROJECT show dsongcpUS.flights_raw || bq --project_id $PROJECT rm --table --force ${PROJECT}:dsongcpUS.flights_raw

# Crear el dataset si es que no existe
bq --project_id=$PROJECT show dsongcpUS || bq --location=US --synchronous_mode mk --dataset dsongcpUS
