...
def urlopen(url):
    '''
    Abre una URL con un contexto SSL personalizado que deshabilita la verificación de certificados.

    Esta función es similar a `urllib.request.urlopen()`, pero permite ignorar los errores de
    verificación de certificados. Úselo con precaución, ya que puede exponer la comunicación a
...
def download(year: str, month: str, destdir: str):
    ...
    Args:
        year (str): Año a procesar e.g.'2015'
        month (str): Mes a procesar e.g. '01 para Enero
    Returns:
        filename (str): Nombre del archivo descargado
    '''
    ...
    url = os.path.join(SOURCE,
                       ...
                       _1987_present_{year}_{int(month)}.zip")
    logging.debug("Trying to download %s", url)

    filename = os.path.join(destdir, "%s%s.zip", year, month)
...
def zip_to_csv(filename, destdir):
    ...
    zip_ref.close()
    logging.info("Extracted %s", csvfile)
    ...
    logging.info("Compressed into %s", gzipped)
...
def upload(csvfile, bucketname, blobname):
    ...
    Args:
        csvfile -- _description_
        bucketname -- _description_
        blobname -- _description_
    ...
    logging.debug('Uploading %s ...', csvfile)
...
def bqload(gcsfile, year, month):
    '''
    Loads the CSV file in GCS into BigQuery, replacing the existing data in \
        that partition
    '''
    ...
    if load_job.state != 'DONE':
        raise load_job.exception()  # type: ignore
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='ingesta de datos de vuelos desde el sitio web de BTS a Google Cloud Storage')
    parser.add_argument(
        '--bucket', help='GCS bucket donde cargar los datos', required=True)
    parser.add_argument(
        '--year', help='Ejemplo: 2015.  Si no se indica, se obtiene por defecto el mes siguiente.')
    parser.add_argument(
