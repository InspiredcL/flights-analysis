...
def urlopen(url):
    ...
    Returns:
        Un objeto similar a un archivo que se puede usar para leer la respuesta.
    ...
    try:
        ...
        raise URLError(f"Error al abrir la URL '{url}': {e}")
...
def bqload(gcsfile, year, month):
    ...
    table_ref = client.dataset('dsongcp').table('flights_auto${}{}'.format(year, month))
    job_config = bigquery.LoadJobConfig()
    job_config.source_format = 'CSV'
    job_config.write_disposition = 'WRITE_TRUNCATE'
    job_config.ignore_unknown_values = True
    job_config.time_partitioning = bigquery.TimePartitioning('MONTH', 'FlightDate')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--debug', dest='debug', action='store_true', help='Debug message')
