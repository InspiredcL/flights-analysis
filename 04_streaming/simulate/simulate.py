...
import datetime
import pytz
from google.cloud import pubsub_v1  # Use v1 of the API
...
def publish(publisher, topics, allevents, notify_time):
    ...
    for key in topics:  # 'departed', 'arrived', etc.
        ...
        logging.info('Publishing %d %s till %s',
...
def notify(publisher, topics, rows, simStartTime, programStart, speedFactor):
    ...
    # registros sobrantes; notificar de nuevo
    publish(publisher, topics, tonotify, notify_time)
...
if __name__ == '__main__':
    ...
    bqclient = bigquery.Client(args.project)
    ...
    job_config = bigquery.QueryJobConfig(
        query_parameters=[
            bigquery.ScalarQueryParameter("jitter", "INT64", jitter),
            bigquery.ScalarQueryParameter(
                "startTime", "TIMESTAMP", args.startTime),
            bigquery.ScalarQueryParameter(
                "endTime", "TIMESTAMP", args.endTime),
        ]
    )
    ...
    for event_type in ['wheelsoff', 'arrived', 'departed']:
        topics[event_type] = publisher_client.topic_path(args.project, event_type)
        try:
            publisher_client.get_topic(topic=topics[event_type])
            logging.info("Already exists: {}".format(topics[event_type]))
        except:
            logging.info("Creating {}".format(topics[event_type]))
            publisher_client.create_topic(name=topics[event_type])

    # notify about each row in the dataset
    programStartTime = datetime.datetime.now(tz=pytz.UTC)
    ...
    notify(publisher_client, topics, rows, simStartTime,
