...
* `-b`, `--bucket`: Bucket de Google Cloud Storage donde se encuentra el
archivo `gs://BUCKET/flights/airports/airports.csv.gz`.
* `-r`, `--region`: Región de Google Cloud en la que se ejecutará el
trabajo de Dataflow. Debe ser la misma región que la del bucket.
...
def airport_timezone(airport_id, airport_timezones):
    ...
    **Excepción:**
    * Si el identificador del aeropuerto no se encuentra en el diccionario
    `airport_timezones`,la función devuelve valores predeterminados
    para la latitud (37.41), longitud (-92.35) y
    ...
    **Ejecución independiente:**
    ...
    with beam.Pipeline(argv=argv) as pipeline:
        ...
        flights_schema = ",".join(
            [
                # Originales
                "FL_DATE:timestamp,UNIQUE_CARRIER:string,"
                "ORIGIN_AIRPORT_SEQ_ID:string,ORIGIN:string",
                "DEST_AIRPORT_SEQ_ID:string,DEST:string,"
                "CRS_DEP_TIME:timestamp,DEP_TIME:timestamp",
                "DEP_DELAY:float,TAXI_OUT:float,WHEELS_OFF:timestamp,"
                "WHEELS_ON:timestamp,TAXI_IN:float",
                "CRS_ARR_TIME:timestamp,ARR_TIME:timestamp,ARR_DELAY:float,"
                "CANCELLED:boolean",
